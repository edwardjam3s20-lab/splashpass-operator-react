...
export function LoginScreen() {
  ...
  useEffect(() => {
    if (operator) navigate('/app/home', { replace: true })
  }, [operator, navigate])
    ...
    <div className="flex h-full w-full items-center justify-center bg-bg px-6">
      <div className="w-full max-w-sm">
        <div className="mb-8 flex items-center justify-center gap-2.5">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gold font-display text-lg font-extrabold text-bg">
            S
          </div>
        ...
        <div className="rounded-[20px] border border-white/[0.07] bg-white/[0.03] p-6">
          ...
          {error && (
            <div className="mb-4 rounded-xl border border-danger/20 bg-danger/10 px-3.5 py-2.5 text-[13px] leading-relaxed text-danger">
              {error}
            </div>
          )}
          ...
          <button
            ...
            disabled={loading}
            onClick={handleLogin}
            className="w-full rounded-xl bg-gold py-3.5 text-sm font-bold text-bg disabled:opacity-50"
          >
